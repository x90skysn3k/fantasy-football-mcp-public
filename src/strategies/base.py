...
class WeightAdjustment(BaseModel):
    ...
    floor_weight: Decimal = Field(Decimal('0.0'), ge=0, le=1, description="Weight for floor projection")
    ceiling_weight: Decimal = Field(Decimal('0.0'), ge=0, le=1, description="Weight for ceiling projection")
    projection_weight: Decimal = Field(Decimal('1.0'), ge=0, le=1, description="Weight for base projection")
    
    # Value and efficiency weights
    value_weight: Decimal = Field(Decimal('0.0'), ge=0, le=1, description="Weight for salary value")
    ownership_weight: Decimal = Field(Decimal('0.0'), ge=-1, le=1, description="Weight for ownership (negative = contrarian)")
    
    # Risk and variance weights
    consistency_weight: Decimal = Field(Decimal('0.0'), ge=0, le=1, description="Weight for consistency/low variance")
    upside_weight: Decimal = Field(Decimal('0.0'), ge=0, le=1, description="Weight for upside potential")
    
    # Matchup-specific weights
    matchup_weight: Decimal = Field(Decimal('0.0'), ge=0, le=1, description="Weight for matchup favorability")
    game_script_weight: Decimal = Field(Decimal('0.0'), ge=0, le=1, description="Weight for game script predictions")
    
    # Position-specific modifiers
    position_modifiers: Dict[str, Decimal] = Field(
        default_factory=dict, 
        description="Position-specific weight modifiers"
    )
...
class StrategyConfig(BaseModel):
    ...
    risk_tolerance: Decimal = Field(
        ...
        ge=0, 
        le=1, 
        description="Risk tolerance (0=ultra-safe, 1=max-risk)"
    )
    ...
    stack_preference: Decimal = Field(
        ...
        ge=0, 
        le=1, 
        description="Preference for team stacking"
    )
    correlation_bonus: Decimal = Field(
        Decimal('0.0'), 
        ge=0, 
        le=1, 
        description="Bonus for positively correlated players"
    )
    ...
    weather_penalty: Decimal = Field(
        ...
        ge=0, 
        le=1, 
        description="Penalty for adverse weather conditions"
    )
...
class BaseLineupStrategy(ABC):
    ...
    def _validate_config(self) -> None:
        ...
        total_core_weights = (
            weights.floor_weight + 
            weights.ceiling_weight + 
    ...
    def calculate_matchup_bonus(
        ...
        if player_team_analysis:
            ...
            if any("favorable" in matchup.lower() for matchup in player_team_analysis.favorable_matchups):
                bonus += Decimal('0.1')
            
            # Penalty for players in concerning matchups
            if any("concern" in matchup.lower() for matchup in player_team_analysis.concerning_matchups):
                bonus -= Decimal('0.1')
    ...
    def calculate_game_script_adjustment(
        ...
        win_probability = (
            matchup_analysis.home_win_probability if is_home_team 
