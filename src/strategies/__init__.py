...
def get_strategy_for_contest_type(contest_type: str) -> StrategyType:
    ...
    if any(keyword in contest_lower for keyword in ['gpp', 'tournament', 'millionaire', 'large', 'high-stakes']):
        return StrategyType.AGGRESSIVE
    
    # Cash games -> Conservative  
    if any(keyword in contest_lower for keyword in ['cash', '50/50', 'double', 'head-to-head', 'safe']):
