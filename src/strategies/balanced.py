...
class BalancedStrategy(BaseLineupStrategy):
    ...
    def score_player(
        ...
        if player.value_metrics and weights.value_weight > 0:
            ...
            if projected_value:
                ...
                value_component = (
                    min(projected_value * Decimal("1.1"), Decimal("11")) * weights.value_weight
                )
        ...
        if (
            ...
            and player.get_projected_value() > Decimal("5.5")
        ):
    ...
    def _calculate_consistency_score(self, player: Player) -> Decimal:
        ...
        if player.projections.floor_points:
            floor_ratio = (
                player.projections.floor_points / player.projections.projected_fantasy_points
            )
    ...
    def _calculate_upside_score(self, player: Player) -> Decimal:
        ...
        if (
            ...
            and player.last_game_stats.fantasy_points
            > player.projections.projected_fantasy_points * Decimal("1.3")
        ):
    ...
    def _apply_balanced_adjustments(
        ...
        if player.is_injured():
            if (
                ...
                and player.injury_report.severity_score >= 3
            ):
                penalty = min(
                    Decimal("0.15"), player.injury_report.severity_score * Decimal("0.02")
                )
                adjustment -= penalty
        ...
        if matchup_analysis and matchup_analysis.matchup.get_total_projected_points():
        ...
        if matchup_analysis and matchup_analysis.key_injuries:
            # Check if key injuries create opportunity
            position_injuries = [
                inj
                for inj in matchup_analysis.key_injuries
        ...
        if (
            ...
            and matchup_analysis.matchup.game_environment.venue_type.value == "Dome"
        ):
            adjustment += Decimal("0.01")
    ...
    def optimize_lineup_balance(
        self, potential_players: List[Player], constraints: Dict
    ) -> Dict[str, Decimal]:
        """
    ...
    def calculate_portfolio_balance(
        self, scored_players: List[Tuple[Player, PlayerScore]]
    ) -> Dict[str, Decimal]:
        """
        ...
        for player, score in scored_players:
            ...
            elif variance >= Decimal("0.7") or (
                score.ceiling_component > score.floor_component * Decimal("1.5")
            ):
    ...
    def _calculate_overall_balance_score(
        self, high_floor_count: int, balanced_count: int, high_upside_count: int
    ) -> Decimal:
    ...
    def _get_balance_recommendations(
        self, high_floor_count: int, balanced_count: int, high_upside_count: int, total_count: int
    ) -> List[str]:
