...
class AggressiveStrategy(BaseLineupStrategy):
    ...
    def score_player(
        ...
        if player.value_metrics and weights.value_weight > 0:
            ...
            if projected_value:
                ...
                value_component = min(projected_value * Decimal('1.2'), Decimal('12')) * weights.value_weight
        ...
        game_script_adjustment = self.calculate_game_script_adjustment(player, matchup_analysis) * Decimal('1.5')
        ...
        if player.value_metrics and player.get_projected_value() and player.get_projected_value() > Decimal('6'):
    ...
    def _calculate_upside_score(self, player: Player) -> Decimal:
        ...
        if (player.last_game_stats and 
            ...
            player.last_game_stats.fantasy_points > player.projections.projected_fantasy_points * Decimal('1.2')):
            upside_score += Decimal('0.1')
    ...
    def _apply_aggressive_adjustments(
        ...
        if (player.last_game_stats and 
            player.last_game_stats.fantasy_points and 
            player.last_game_stats.fantasy_points >= Decimal('20')):
        ...
        if (matchup_analysis and 
            matchup_analysis.matchup.get_total_projected_points() and
            matchup_analysis.matchup.get_total_projected_points() >= Decimal('50')):
        ...
        if (matchup_analysis and 
            matchup_analysis.volatility_rating >= 7):
            adjustment += Decimal('0.05')
        ...
        if player.season_stats and player.position in [Position.WR, Position.TE]:
            ...
            if (player.season_stats.targets and 
                player.season_stats.targets / games_played >= 6 and
                player.season_stats.targets / games_played <= 10):
                adjustment += Decimal('0.04')
        ...
        if (matchup_analysis and 
            ...
            matchup_analysis.matchup.game_environment.is_weather_concern()):
        ...
        if (player.value_metrics and 
            player.value_metrics.projected_ownership and
            player.value_metrics.projected_ownership > Decimal('30') and
            player.projections and
            player.projections.projected_fantasy_points < Decimal('20')):
        ...
        if player.news_notes:
            ...
            if any(keyword in note.lower() for note in player.news_notes for keyword in narrative_keywords):
        ...
        if player.position == Position.QB:
            ...
            if (player.season_stats and 
                player.season_stats.rushing_attempts and 
                player.season_stats.games_played and
                player.season_stats.rushing_attempts / player.season_stats.games_played >= 5):
                adjustment += Decimal('0.05')
            ...
            if (player.season_stats and 
                player.season_stats.targets and 
                player.season_stats.games_played and
                player.season_stats.targets / player.season_stats.games_played >= 4):
                adjustment += Decimal('0.03')
            ...
            if (player.season_stats and 
                player.season_stats.receiving_yards and 
                player.season_stats.receptions and 
                player.season_stats.receptions > 0):
                yards_per_reception = player.season_stats.receiving_yards / player.season_stats.receptions
    ...
    def get_leverage_opportunities(
        self, 
        players: List[Player], 
        matchup_analyses: Optional[List[MatchupAnalysis]] = None
    ) -> List[Dict]:
        """
        ...
        for player in players:
            ...
            if (player.last_game_stats and 
                player.last_game_stats.fantasy_points and 
                player.last_game_stats.fantasy_points < Decimal('8') and
                player.projections.projected_fantasy_points >= Decimal('12')):
