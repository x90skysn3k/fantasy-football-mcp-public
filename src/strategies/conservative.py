...
class ConservativeStrategy(BaseLineupStrategy):
    ...
    def score_player(
        ...
        if player.value_metrics and player.get_projected_value() and player.get_projected_value() > Decimal('5'):
            boost_factors.append("Excellent value play")
        
        # Add general penalty factors
        if player.projections.bust_probability and player.projections.bust_probability > Decimal('0.3'):
    ...
    def _calculate_consistency_score(self, player: Player) -> Decimal:
        ...
        if player.projections.floor_points and player.projections.ceiling_points:
            ...
            if projection_range > 0:
                ...
                consistency_boost = max(Decimal('0'), (Decimal('1') - variance_ratio)) * Decimal('0.3')
    ...
    def _apply_conservative_adjustments(
        ...
        if player.is_injured():
            ...
            if (player.injury_report and 
                ...
                player.injury_report.severity_score >= 5):
        ...
        if player.season_stats and player.position in [Position.WR, Position.TE, Position.RB]:
            ...
            if player.position in [Position.WR, Position.TE]:
                if (player.season_stats.targets and 
                    player.season_stats.targets / games_played >= 8):
                    adjustment += Decimal('0.03')
                    boost_factors.append("High target share")
            elif player.position == Position.RB:
                if (player.season_stats.rushing_attempts and 
                    player.season_stats.rushing_attempts / games_played >= 15):
                    adjustment += Decimal('0.03')
        ...
        if (matchup_analysis and 
            ...
            matchup_analysis.matchup.game_environment.venue_type.value in ["Dome", "Retractable"]):
