...
class LineupChromosome:
    ...
    def _get_player_salary(self, player: Player) -> int:
        ...
        if player.value_metrics:
            return (player.value_metrics.draftkings_salary or
                   player.value_metrics.fanduel_salary or
                   player.value_metrics.yahoo_salary or
                   5000)
        return 5000
...
class OptimizationAgent:
    ...
    async def rank_waiver_targets(
        ...
        for player in available_players:
            task = self._evaluate_waiver_impact(
                player, current_roster, constraints, weeks_ahead
            )
            tasks.append(task)
    ...
    async def _filter_valid_players(
        ...
        async def is_player_valid(player: Player) -> bool:
            ...
            # Check exclusions
            if (constraints.excluded_players and 
                player.id in constraints.excluded_players):
                return False
            
            # Check salary constraints
            if player.value_metrics:
                salary = (player.value_metrics.draftkings_salary or
                         player.value_metrics.fanduel_salary or
                         player.value_metrics.yahoo_salary)
    ...
    async def _evaluate_waiver_impact(
        ...
        try:
            ...
            # Calculate impact
            point_improvement = (with_player_lineup.total_projected_points - 
    ...
    async def _find_position_replacements(
        ...
        for replacement in position_matches:
            task = self._evaluate_replacement_player(
                injured_player, replacement, constraints
            )
            tasks.append(task)
    ...
    def _group_players_by_position(self, players: List[Player]) -> Dict[Position, List[Player]]:
        ...
        for position in groups:
            groups[position].sort(
                key=lambda p: p.projections.projected_fantasy_points if p.projections else Decimal('0'),
                reverse=True
            )
    ...
    async def _generate_position_combinations(
        self,
        players_by_position: Dict[Position, List[Player]],
        constraints: LineupConstraints
    ) -> List[List[Player]]:
        ...
        qb_players = players_by_position.get(Position.QB, [])[:max_players_per_position[Position.QB]]
        rb_players = players_by_position.get(Position.RB, [])[:max_players_per_position[Position.RB]]
        wr_players = players_by_position.get(Position.WR, [])[:max_players_per_position[Position.WR]]
        te_players = players_by_position.get(Position.TE, [])[:max_players_per_position[Position.TE]]
        def_players = players_by_position.get(Position.DEF, [])[:max_players_per_position[Position.DEF]]
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for core_combo in core_combinations[:10000]:  # Limit for performance
                ...
                for flex_player in available_flex[:3]:  # Top 3 FLEX options
                    lineup_players = list(qbs) + list(rbs) + list(wrs) + list(tes) + list(defs) + [flex_player]
    ...
    def _create_lineup_from_players(
        ...
        position_map = [
            ...
            Position.WR, Position.WR, Position.TE, Position.RB, Position.DEF
        ]
    ...
    async def _create_random_valid_lineup(
        ...
        for position in positions_needed:
            ...
            affordable_players = [
                ...
                self._get_player_salary_for_optimization(p) <= remaining_salary
            ]
            ...
            if not affordable_players:
                ...
                if affordable_players:
                    affordable_players = [min(affordable_players, 
    ...
    async def _crossover(
        self,
        parent1: LineupChromosome,
        parent2: LineupChromosome,
        constraints: LineupConstraints
    ) -> LineupChromosome:
    ...
    async def _mutate(
        ...
        try:
            ...
            for pos_idx in positions_to_mutate:
                ...
                position_players = [p for p in players if p.position == position and p.id != current_player.id]
                
                if position_players:
                    # Filter by salary constraint
                    current_salary = sum(self._get_player_salary_for_optimization(p) for p in mutated_players)
                    current_player_salary = self._get_player_salary_for_optimization(current_player)
                    available_salary = constraints.salary_cap - current_salary + current_player_salary
    ...
    async def _create_alternative_lineup(
        ...
        try:
            ...
            modified_constraints = LineupConstraints(
                ...
                excluded_players=(constraints.excluded_players or []) + [p.id for p in optimal_lineup.get_players()[:3]]
            )
            
            alternative_lineup = await self._quick_optimization(players, modified_constraints, objective)
            
            if alternative_lineup:
                # Calculate differences
                point_diff = alternative_lineup.total_projected_points - optimal_lineup.total_projected_points
                salary_diff = alternative_lineup.total_salary - optimal_lineup.total_salary
                ownership_diff = (alternative_lineup.projected_ownership or Decimal('50')) - (optimal_lineup.projected_ownership or Decimal('50'))
    ...
    async def _greedy_optimization(
        ...
        if objective == OptimizationObjective.MAXIMIZE_POINTS:
            players.sort(key=lambda p: p.projections.projected_fantasy_points if p.projections else Decimal('0'), reverse=True)
        elif objective == OptimizationObjective.MAXIMIZE_VALUE:
            players.sort(key=lambda p: p.get_projected_value() or Decimal('0'), reverse=True)
        elif objective == OptimizationObjective.MINIMIZE_OWNERSHIP:
            players.sort(key=lambda p: p.value_metrics.ownership_percentage if p.value_metrics else Decimal('50'))
        ...
        for position in positions_needed:
            ...
            for player in players:
                if (player.position == position and
                    player not in selected_players and
                    self._get_player_salary_for_optimization(player) <= remaining_salary):
    ...
    async def _evaluate_replacement_player(
        ...
        injured_points = injured_player.projections.projected_fantasy_points if injured_player.projections else Decimal('0')
        replacement_points = replacement.projections.projected_fantasy_points if replacement.projections else Decimal('0')
    ...
    def _calculate_upside_factor(self, player: Player, weeks_ahead: int) -> float:
        ...
        ceiling_projection = float(player.projections.ceiling_points or player.projections.projected_fantasy_points)
    ...
    def _calculate_consistency_factor(self, player: Player) -> float:
        ...
        base_projection = float(player.projections.projected_fantasy_points)
        floor_projection = float(player.projections.floor_points or player.projections.projected_fantasy_points * Decimal('0.7'))
    ...
    def _get_contest_recommendations(self, lineup: Lineup, strategy: OptimizationStrategy) -> List[str]:
