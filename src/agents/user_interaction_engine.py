...
class UserInteractionEngine:
    ...
    async def handle_interaction(self, interaction: UserInteraction) -> InteractionResponse:
        """
        ...
        try:
            ...
            if interaction.interaction_type == InteractionType.WHAT_IF_PLAYER:
                ...
                response = InteractionResponse(
                    answer="I don't understand that type of interaction.", confidence=0.0
                )
            ...
            return InteractionResponse(
                answer=f"I encountered an error processing your request: {str(e)}", confidence=0.0
            )
    ...
    async def _handle_what_if_constraints(
    ...
    async def _handle_explain_decision(self, interaction: UserInteraction) -> InteractionResponse:
        ...
        if not interaction.current_lineup:
            return InteractionResponse(
                answer="I need the current lineup to explain the decisions.", confidence=0.0
            )
    ...
    async def _handle_compare_alternatives(
        self, interaction: UserInteraction
    ) -> InteractionResponse:
        ...
        # Format comparison
        comparison_text = self._format_alternatives_comparison(
            alternatives, interaction.current_lineup
        )
    ...
    async def _handle_analyze_risk(self, interaction: UserInteraction) -> InteractionResponse:
        ...
        if not interaction.current_lineup:
            return InteractionResponse(
                answer="I need the current lineup to analyze risk.", confidence=0.0
            )
    ...
    async def _handle_market_analysis(self, interaction: UserInteraction) -> InteractionResponse:
        ...
        if not interaction.current_lineup:
            return InteractionResponse(
                answer="I need the current lineup to analyze market conditions.", confidence=0.0
            )
    ...
    async def _analyze_player_substitution(
        self, current_lineup: Lineup, target_player: Player, available_players: List[Player]
    ) -> Dict[str, Any]:
        ...
        for slot in current_lineup.slots:
            if (
                slot.player
                and slot.position == target_player.position
                and slot.player.id != target_player.id
            ):
        ...
        current_points = (
            current_player.get_best_projection()
            if hasattr(current_player, "get_best_projection")
            else 0
        )
        target_points = (
            target_player.get_best_projection()
            if hasattr(target_player, "get_best_projection")
            else 0
        )
    ...
    def _format_alternatives_comparison(
        self, alternatives: List[Dict[str, Any]], current_lineup: Lineup
    ) -> str:
    ...
    async def _analyze_improvement_opportunities(
        self, current_lineup: Lineup, available_players: List[Player]
    ) -> Dict[str, Any]:
        ...
        for slot in current_lineup.slots:
            if slot.player:
                current_proj = (
                    slot.player.get_best_projection()
                    if hasattr(slot.player, "get_best_projection")
                    else 0
                )
                ...
                better_players = [
                    ...
                    if (
                        ...
                        and p.id != slot.player.id
                        and p.get_best_projection() > current_proj
                    )
                ]
    ...
    async def _perform_risk_analysis(self, current_lineup: Lineup) -> Dict[str, Any]:
        ...
        injured_players = [
            slot.player for slot in current_lineup.slots if slot.player and slot.player.is_injured()
        ]
        if injured_players:
        ...
        high_owned = [
            ...
            if (
                slot.player
                and slot.player.value_metrics
                and slot.player.value_metrics.projected_ownership
                and slot.player.value_metrics.projected_ownership > 30
            )
        ]
    ...
    def _update_interaction_stats(
        self, start_time: datetime, success: bool, interaction_type: InteractionType
    ):
