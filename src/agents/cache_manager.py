...
class CacheManagerAgent:
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            memory_usage_pct = (self._current_memory_size / self.max_memory_size * 100) if self.max_memory_size > 0 else 0
            
            stats_dict.update({
                'memory_cache_size': len(await self._memory_cache.keys()) if self._memory_cache else 0,
    ...
    async def _enforce_memory_limits(self, new_entry_size: int = 0) -> None:
        ...
        while self._current_memory_size > target_size and self._lru_order:
            ...
            if self.eviction_policy == EvictionPolicy.LRU:
                ...
                largest_key = max(
                    self._entries.keys(),
                    key=lambda k: self._entries[k].size_bytes
                )
    ...
    def get_memory_usage(self) -> Dict[str, Any]:
        ...
        with self._thread_lock:
            usage_pct = (self._current_memory_size / self.max_memory_size * 100) if self.max_memory_size > 0 else 0
            ...
            return {
                ...
                'avg_entry_size': round(self._current_memory_size / len(self._entries), 2) if self._entries else 0
            }