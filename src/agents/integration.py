...
from .user_interaction_engine import (
...
class FantasyFootballAssistant:
    ...
    def __init__(self, llm_api_key: str, max_workers: int = None, enable_llm: bool = True):
        ...
        self.hybrid_optimizer = HybridLineupOptimizer(
            llm_api_key=llm_api_key, max_workers=max_workers, enable_llm=enable_llm
        )
    ...
    async def ask_question(
        self, question: str, interaction_type: Optional[InteractionType] = None
    ) -> InteractionResponse:
        """
        ...
        try:
            ...
            return InteractionResponse(
                answer=f"I encountered an error processing your question: {str(e)}", confidence=0.0
            )
    ...
    async def analyze_edge_cases(self) -> List[Any]:
        ...
        try:
            return await self.hybrid_optimizer.analyze_edge_cases(
                self.current_lineup, self.available_players, self.session_context
            )
        except Exception as e:
            self.logger.error(f"Edge case analysis failed: {e}")
            return []

    async def adapt_strategy(self, market_conditions: Dict[str, Any]) -> Dict[str, Any]:
        """Adapt strategy based on current market conditions."""
        try:
            return await self.hybrid_optimizer.adapt_strategy(
                self.current_strategy, market_conditions, self.session_context
            )
        except Exception as e:
    ...
    def _detect_interaction_type(self, question: str) -> InteractionType:
        ...
        if any(
            ...
            for phrase in ["what if i use", "instead of", "replace", "substitute", "switch to"]
        ):
        ...
        if any(
            phrase in question_lower
            for phrase in ["strategy", "approach", "optimize for", "focus on"]
        ):
            return InteractionType.WHAT_IF_STRATEGY

        # Constraint questions
        if any(phrase in question_lower for phrase in ["salary", "budget", "constraint", "limit"]):
            return InteractionType.WHAT_IF_CONSTRAINTS

        # Explanation questions
        if any(
            phrase in question_lower
            for phrase in ["explain", "why", "reason", "rationale", "decision"]
        ):
        ...
        if any(
            phrase in question_lower
            for phrase in ["compare", "alternative", "different", "other options"]
        ):
            return InteractionType.COMPARE_ALTERNATIVES

        # Improvement questions
        if any(phrase in question_lower for phrase in ["improve", "better", "optimize", "enhance"]):
            return InteractionType.SUGGEST_IMPROVEMENTS

        # Risk questions
        if any(phrase in question_lower for phrase in ["risk", "safe", "dangerous", "volatile"]):
            return InteractionType.ANALYZE_RISK

        # Market questions
        if any(
            phrase in question_lower for phrase in ["market", "ownership", "popular", "contrarian"]
        ):
    ...
    def get_session_summary(self) -> Dict[str, Any]:
        ...
        session_duration = (
            datetime.now() - self.session_stats["session_start_time"]
        ).total_seconds()
...
async def create_fantasy_assistant(
    llm_api_key: str, max_workers: int = None, enable_llm: bool = True
...
async def quick_optimize(
    ...
    assistant = await create_fantasy_assistant(
        llm_api_key=llm_api_key or "", enable_llm=llm_api_key is not None
    )

    return await assistant.optimize_lineup(
        players=players, constraints=constraints, strategy=strategy, context=context
    )
...
async def example_usage():
    ...
    sample_players = [
        Player(id="1", name="Josh Allen", position=Position.QB, team=Team.BUF, season=2024, week=1),
        # ... more players
    ]
    ...
    constraints = LineupConstraints(
        salary_cap=50000,
        position_requirements={"QB": 1, "RB": 2, "WR": 2, "TE": 1, "FLEX": 1, "K": 1, "DEF": 1},
    )

    # Create assistant (you would provide your actual API key)
    assistant = await create_fantasy_assistant(llm_api_key="your-api-key-here", enable_llm=True)
    ...
    response = await assistant.ask_question("What if I use CMC instead of the current RB?")
