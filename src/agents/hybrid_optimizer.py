...
class HybridOptimizationResult:
    ...
    def get_risk_assessment(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "mitigation_strategies": self.llm_analysis.risk_assessment.get("mitigation_strategies", []),
            "confidence": self.overall_confidence
        }
...
class HybridLineupOptimizer:
    ...
    async def optimize_lineup(
        ...
        try:
            ...
            if use_llm and self.llm_engine:
                try:
                    llm_analysis, alternatives, edge_cases, strategy_adaptations = await self._perform_llm_enhancement(
                        math_result, players, strategy, constraints, context
                    )
                    self.optimization_stats['llm_enhancements'] += 1
                except Exception as e:
                    self.logger.warning(f"LLM enhancement failed: {e}, continuing with math-only result")
            ...
            self.logger.info(f"Hybrid optimization completed successfully - "
    ...
    async def analyze_edge_cases(
        ...
        try:
            return await self.llm_engine.handle_edge_cases(
                lineup, players, enhancement_context
            )
        except Exception as e:
            ...
            "optimal_lineup": {
                ...
                "starters": {
                    slot.position.value: {
                        ...
                        "projected_points": float(slot.player.projections.projected_fantasy_points) if slot.player and slot.player.projections else 0.0,
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        if stats['total_optimizations'] > 0:
            ...
            stats['math_only_fallback_rate'] = stats['math_only_fallbacks'] / stats['total_optimizations']
        else:
...
async def optimize_lineup_hybrid(
    ...
    optimizer = HybridLineupOptimizer(
        ...
        enable_llm=use_llm and llm_api_key is not None
    )
