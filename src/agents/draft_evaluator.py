...
class DraftEvaluatorAgent:
    ...
    def __init__(
        self, settings: Settings, cache_manager: CacheManager, data_fetcher: DataFetcherAgent
    ):
    ...
    async def get_draft_recommendation(
        ...
        try:
            ...
            top_evaluations = sorted(evaluations, key=lambda x: x.overall_score, reverse=True)[
                :num_recommendations
            ]
    ...
    async def analyze_draft_state(
        self, league_key: str, strategy: DraftStrategy = DraftStrategy.BALANCED
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            analysis = {
                ...
                "strategic_recommendations": await self._get_phase_strategy_recommendations(
                    draft_state
                ),
    ...
    async def _build_draft_state(
        self, league_key: str, strategy: DraftStrategy, current_pick: Optional[int] = None
    ) -> DraftState:
        ...
        if current_pick:
            draft_pos = self._calculate_draft_position(
                current_pick, league_info.get("num_teams", 12)
            )
        else:
            draft_pos = DraftPosition(
                overall_pick=len(current_roster) + 1,
                round_number=math.ceil(
                    (len(current_roster) + 1) / league_info.get("num_teams", 12)
                ),
    ...
    async def _evaluate_available_players(
        self, available_players: List[Player], draft_state: DraftState, current_roster: List[Player]
    ...
    async def _calculate_positional_scarcity(
        self, player: Player, draft_state: DraftState
    ) -> float:
    ...
    async def _create_recommendation(
        self, evaluation: PlayerEvaluation, rank: int
    ) -> DraftRecommendation:
    ...
    async def _generate_strategic_insights(
        self, draft_state: DraftState, recommendations: List[DraftRecommendation]
    ) -> Dict[str, Any]:
        """Generate strategic insights and analysis."""

        insights = {"key_insights": [], "positional_priorities": {}, "risk_factors": []}

        # Key insights based on draft phase
        if draft_state.draft_phase == "early":
            insights["key_insights"].append(
                "Early draft: Prioritize best player available and elite tier players"
            )
        elif draft_state.draft_phase == "middle":
            insights["key_insights"].append("Mid draft: Balance positional needs with player value")
        else:
            insights["key_insights"].append(
                "Late draft: Focus on upside players and bye week management"
            )

        # Positional priority analysis
        critical_needs = [
            need for need in draft_state.roster_needs if need.need_level == PositionalNeed.CRITICAL
        ]
        if critical_needs:
        ...
        for need in draft_state.roster_needs:
            ...
            insights["positional_priorities"][str(need.position.value)] = priority_scores[
                need.need_level
            ]
