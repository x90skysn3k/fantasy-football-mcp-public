...
class HybridOptimizerConfig:
    ...
    def from_environment(cls) -> 'HybridOptimizerConfig':
        ...
        config.optimization.use_genetic_algorithm = os.getenv('USE_GENETIC_ALGORITHM', 'true').lower() == 'true'
        config.optimization.genetic_population_size = int(os.getenv('GENETIC_POPULATION_SIZE', '1000'))
        config.optimization.genetic_generations = int(os.getenv('GENETIC_GENERATIONS', '200'))
        
        # Enhancement Configuration
        config.enhancement.enable_explanations = os.getenv('ENABLE_EXPLANATIONS', 'true').lower() == 'true'
        config.enhancement.enable_alternatives = os.getenv('ENABLE_ALTERNATIVES', 'true').lower() == 'true'
        config.enhancement.enable_edge_case_handling = os.getenv('ENABLE_EDGE_CASES', 'true').lower() == 'true'
        ...
        config.logging.enable_file_logging = os.getenv('ENABLE_FILE_LOGGING', 'false').lower() == 'true'
        config.logging.log_file_path = os.getenv('LOG_FILE_PATH', 'fantasy_optimizer.log')
        
        # Feature Flags
        config.enable_llm_enhancement = os.getenv('ENABLE_LLM_ENHANCEMENT', 'true').lower() == 'true'
    ...
    def validate(self) -> List[str]:
        ...
        if self.optimization.genetic_population_size < 10 or self.optimization.genetic_population_size > 10000:
            issues.append("Genetic population size must be between 10 and 10000")
        
        if self.optimization.genetic_generations < 1 or self.optimization.genetic_generations > 1000:
            issues.append("Genetic generations must be between 1 and 1000")
        
        if self.optimization.genetic_mutation_rate < 0 or self.optimization.genetic_mutation_rate > 1:
            issues.append("Genetic mutation rate must be between 0 and 1")
        
        if self.optimization.genetic_crossover_rate < 0 or self.optimization.genetic_crossover_rate > 1:
        ...
        if self.performance.metrics_retention_days < 1 or self.performance.metrics_retention_days > 365:
