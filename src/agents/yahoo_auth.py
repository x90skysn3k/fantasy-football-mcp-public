...
class YahooAuth:
    ...
    def is_authenticated(self) -> bool:
        ...
        return (
            ...
            and self.auth_state == AuthState.AUTHENTICATED
        )
    ...
    def get_authorization_url(
        self, state: Optional[str] = None, scopes: str = DEFAULT_SCOPES
    ) -> Tuple[str, str]:
        """
    ...
    def _start_callback_server(self) -> None:
        ...
        try:
            self._auth_server = HTTPServer(
                (self.callback_host, self.callback_port), callback_handler_factory
            )

            # Start server in separate thread
            server_thread = threading.Thread(target=self._auth_server.serve_forever, daemon=True)
            server_thread.start()
    ...
    async def authenticate(
        self, timeout: int = 300, scopes: str = DEFAULT_SCOPES, auto_open_browser: bool = True
    ) -> YahooTokens:
        """
...
async def quick_authenticate(
    settings: Settings, timeout: int = 300, auto_open_browser: bool = True
) -> YahooTokens:
    """
    ...
    """
    auth = YahooAuth(settings)
    return await auth.authenticate(timeout=timeout, auto_open_browser=auto_open_browser)
