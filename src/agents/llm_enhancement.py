...
class LLMEnhancementEngine:
    ...
    async def handle_edge_cases(
        self, lineup: Lineup, players: List[Player], context: EnhancementContext
    ) -> List[LLMInsight]:
        """Detect and handle edge cases in lineup construction."""
        return await self.edge_case_handler.detect_and_handle_edge_cases(lineup, players, context)
...
"""
    ...
    async def _add_specialized_insights(
        self, analysis: LLMAnalysis, analysis_data: Dict[str, Any]
    ) -> LLMAnalysis:
...
"""
    ...
    def _format_lineup_for_explanation(self, lineup: Lineup) -> str:
        ...
        for slot in lineup.slots:
            if slot.player:
                formatted.append(
                    f"{slot.position.value}: {slot.player.name} ({slot.player.team.value})"
                )
        return "\n".join(formatted)
...
class EdgeCaseHandler:
    ...
    def _check_weather_concerns(
        self, lineup: Lineup, context: EnhancementContext
    ) -> List[LLMInsight]:
    ...
    def _check_game_script_concerns(
        self, lineup: Lineup, context: EnhancementContext
    ) -> List[LLMInsight]:
