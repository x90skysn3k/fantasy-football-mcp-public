...
class RosterConfiguration:
    ...
    def detect_league_type(cls, roster_positions: List[RosterPosition]) -> LeagueType:
        ...
        superflex_count = sum(
            p.count for p in roster_positions if p.position_type in ["SUPERFLEX", "OP"]
        )
        idp_count = sum(
            p.count for p in roster_positions if p.position_type in ["LB", "DB", "DL", "IDP"]
        )
    ...
    @classmethod
    def validate_lineup(
        cls, lineup: List[Dict[str, str]], roster_positions: List[RosterPosition]
    ) -> Tuple[bool, List[str]]:
        """
    ...
    @classmethod
    def can_slot_player(
        cls, player_position: str, roster_slot: str, roster_positions: List[RosterPosition]
    ) -> bool:
        """
    ...
    @classmethod
    def optimize_position_assignment(
        cls, players: List[Dict], roster_positions: List[RosterPosition]
    ) -> List[Dict]:
        """
        ...
        for position in roster_positions:
            ...
            if not position.eligible_positions:  # Non-flex position
                ...
                for i in range(min(position.count, len(eligible_players))):
                    assignments.append(
                        {"player": eligible_players[i], "slot": position.position_type}
                    )
