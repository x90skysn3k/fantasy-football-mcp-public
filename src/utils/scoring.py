...
def calculate_fantasy_points(
    ...
    # Kicking points
    if position == "K" and any([stats.pat_made, stats.fg_made_0_39, stats.fg_made_40_49, stats.fg_made_50_plus]):
...
def project_points(
    ...
    base_distribution = stats.norm(
        loc=projection_input.mean_points,
        scale=projection_input.std_dev
    )
...
def position_scarcity_multiplier(
    ...
    if replacement_level is None:
        ...
        replacement_level = weekly_rankings[replacement_idx] if replacement_idx < len(weekly_rankings) else 0
...
def calculate_game_environment_impact(
    ...
    return {
        ...
        "pace_category": "fast" if pace_factor > 1.1 else "slow" if pace_factor < 0.9 else "average"
    }