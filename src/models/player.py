...
class PlayerProjections(BaseModel):
    ...
    confidence_score: Decimal = Field(
        ..., ge=0, le=1, description="Confidence in projection (0-1 scale)"
    )
    ...
    bust_probability: Optional[Decimal] = Field(
        None, ge=0, le=1, description="Probability of significant underperformance"
    )
...
class InjuryReport(BaseModel):
    ...
    severity_score: Optional[int] = Field(
        None, ge=1, le=10, description="Injury severity (1=minor, 10=season-ending)"
    )
...
class PlayerValue(BaseModel):
    ...
    ownership_percentage: Optional[Decimal] = Field(
        None, ge=0, le=100, description="Public ownership percentage"
    )
    projected_ownership: Optional[Decimal] = Field(
        None, ge=0, le=100, description="Projected ownership percentage"
    )

    # Value metrics
    points_per_dollar: Optional[Decimal] = Field(None, description="Points per salary dollar")
    value_score: Optional[Decimal] = Field(
        None, ge=0, le=10, description="Overall value score (0-10)"
    )
...
class Player(BaseModel):
    ...
    tags: Optional[List[str]] = Field(
        None, description="Player tags (e.g., 'sleeper', 'must-start')"
    )
    ...
    def get_projected_value(self) -> Optional[Decimal]:
        ...
        salary = (
            self.value_metrics.draftkings_salary
            or self.value_metrics.fanduel_salary
            or self.value_metrics.yahoo_salary
        )
