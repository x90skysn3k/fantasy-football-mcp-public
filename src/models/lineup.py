...
class LineupConstraints(BaseModel):
    ...
    min_salary_usage: Optional[Decimal] = Field(
        ...
        ge=0, 
        le=1, 
        description="Minimum salary utilization percentage"
    )
    ...
    max_players_per_team: Optional[int] = Field(
        None, 
        ge=1, 
        description="Maximum players from same NFL team"
    )
    
    # Ownership constraints
    max_total_ownership: Optional[Decimal] = Field(
        None, 
        description="Maximum combined ownership percentage"
    )
    
    # Stack constraints
    required_stacks: Optional[List[str]] = Field(
        None, 
        description="Required player stacks (e.g., QB-WR)"
    )
    avoided_stacks: Optional[List[str]] = Field(
        None, 
        description="Stacks to avoid"
    )
    
    # Exclusions
    excluded_players: Optional[List[str]] = Field(
        None, 
        description="Player IDs to exclude from consideration"
    )
...
class Lineup(BaseModel):
    ...
    variance_score: Optional[Decimal] = Field(
        None, 
        ge=0, 
        description="Lineup variance/volatility score"
    )
    
    # Strategy and optimization
    optimization_strategy: Optional[OptimizationStrategy] = Field(
        None, 
        description="Strategy used to build lineup"
    )
    confidence_score: Decimal = Field(
        ..., 
        ge=0, 
        le=1, 
        description="Confidence in lineup performance"
    )
    ...
    gpp_score: Optional[Decimal] = Field(
        ...
        ge=0, 
        le=10, 
        description="Suitability for GPP contests (0-10)"
    )
    cash_game_score: Optional[Decimal] = Field(
        None, 
        ge=0, 
        le=10, 
        description="Suitability for cash games (0-10)"
    )
    
    # Lineup analysis
    team_stacks: Optional[List[str]] = Field(None, description="Team stacks in lineup")
    correlation_score: Optional[Decimal] = Field(
        None, 
        description="Player correlation score"
    )
    ...
    def validate_against_constraints(self, constraints: LineupConstraints) -> List[str]:
        ...
        if self.total_salary > constraints.salary_cap:
            violations.append(f"Exceeds salary cap by ${self.total_salary - constraints.salary_cap}")
        ...
        if constraints.min_salary_usage:
            ...
            if self.total_salary < min_salary:
                violations.append(f"Under minimum salary usage by ${min_salary - self.total_salary}")
...
class LineupRecommendation(BaseModel):
    ...
    alternatives: List[LineupAlternative] = Field(
        ...
        description="Alternative lineup options"
    )
    ...
    leverage_spots: Optional[List[str]] = Field(
        None, 
        description="Low-owned players with high upside"
    )
    chalk_plays: Optional[List[str]] = Field(
        None, 
        description="High-owned popular players"
    )
    ...
    weather_considerations: Optional[List[str]] = Field(
        None, 
        description="Weather-related considerations"
    )
    game_environment_notes: Optional[List[str]] = Field(
        None, 
        description="Game environment factors"
    )
    ...
    bankroll_percentage: Optional[Decimal] = Field(
        ...
        ge=0, 
        le=1, 
        description="Recommended bankroll allocation"
    )
    ...
    expected_percentile: Optional[Decimal] = Field(
        ...
        ge=0, 
        le=100, 
        description="Expected finish percentile"
    )
    ...
    def get_lowest_owned_lineup(self) -> Lineup:
        ...
        return min(
            all_lineups, 
            key=lambda lineup: lineup.projected_ownership or Decimal('100')
        )
    ...
    def get_safest_lineup(self) -> Lineup:
        ...
        return max(
            all_lineups, 
            key=lambda lineup: lineup.floor_points or Decimal('0')
        )
...
class LineupOptimizer(BaseModel):
    ...
    points_weight: Decimal = Field(Decimal('0.7'), ge=0, le=1, description="Weight for projected points")
    value_weight: Decimal = Field(Decimal('0.2'), ge=0, le=1, description="Weight for salary value")
    ownership_weight: Decimal = Field(Decimal('0.1'), ge=0, le=1, description="Weight for ownership")
    
    # Risk parameters
    variance_penalty: Decimal = Field(Decimal('0.0'), ge=0, description="Penalty for high variance")
    correlation_bonus: Decimal = Field(Decimal('0.0'), ge=0, description="Bonus for positive correlation")
    ...
    randomness_factor: Decimal = Field(
        ...
        ge=0, 
        le=1, 
        description="Randomness in optimization"
    )
    
    diversity_requirement: Optional[int] = Field(
        None, 
        description="Number of diverse lineups to generate"
    )
...
class LineupPool(BaseModel):
    ...
    diversification_score: Optional[Decimal] = Field(
        ...
        ge=0, 
        le=1, 
        description="Diversification across lineups"
    )
    ...
    correlation_matrix: Optional[Dict[str, Dict[str, Decimal]]] = Field(
        None, 
        description="Player correlation matrix for pool"
    )
    ...
    def get_player_exposure(self) -> Dict[str, Decimal]:
        ...
        return {
            player_id: Decimal(count) / total_lineups 
