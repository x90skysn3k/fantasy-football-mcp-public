    ...
    for tool_name, tool_args in tools_to_test:
        try:
            ...
            if isinstance(result, list) and result:
                ...
                players = data.get('all_players', []) or data.get('enhanced_players', []) or data.get('players', []) or data.get('roster', [])
                if players:
                    ...
                    proj_count = sum(1 for p in players if (p.get('sleeper_projection', 0) > 0 or p.get('yahoo_projection', 0) > 0))
                    advice_count = sum(1 for p in players if p.get('expert_advice') and len(p.get('expert_advice', '')) > 10)
